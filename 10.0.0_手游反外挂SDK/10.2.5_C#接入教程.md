...
|参数 | 说明|
|-----|----|
|account_type | 与运营平台相关的账号类型，参考下文的AnoEntry填写|
|open_id | 用户唯一身份标识，可自定义字符串，不要包含特殊字符（\|#-@）、中文，长度限制在64字节。（和处罚相关必须填写）|

account_type默认QQ平台填1，微信平台填2，其他平台填99。国内外主流账号登录平台可参考下面的数值填写。
...
private void Start () {
    int account_type = (int)AnoEntry.ENTRY_ID_QZONE; /*与运营平台相关的账号类型*/
    string open_id = "B73B36366565F9E02C7525516DCF31C2"; /*与登录平台相关的账号id*/
